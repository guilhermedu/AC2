#contador em anel(ring counter) de 4 bits atualizado a uma frequencia de 3Hz
...
main:
    ...
    ori $s1,$s1,0x0002      # Modifica RB3=in(1);
...
loop:
    ...
    andi $s2,$s2,0x0002     # le RB3
if:
    beq $s2,0x0002,else
    srl $s3,$s3,1          #contador >> 1
    j if_reset
else:
    sll $s3,$s3,1          #contador << 1
if_reset:
    ble $s3,0x0008,else_if_reset
    li $s3,0x0001
else_if_reset:
    bgt $s3,0x0001,no_reset
    li $s3,0x0008
